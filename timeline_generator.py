...
def load_linked_sources():
    ...
    return (card_swipes, cctv, wifi_logs, lab_bookings, text_notes, library_checkouts, entity_table)
...
def generate_timeline(entity_id, card_swipes, cctv, wifi_logs, lab_bookings,
    ...
    def prepare_df(df, cols): return df[cols]
    ...
    all_events = pd.concat([card_swipes_tl, cctv_tl, wifi_tl, lab_tl, notes_tl, library_tl], ignore_index=True, sort=False).sort_values('timestamp')
    if start_date: all_events = all_events[all_events['timestamp'] >= pd.to_datetime(start_date)]
    if end_date: all_events = all_events[all_events['timestamp'] <= pd.to_datetime(end_date)]
    if event_types: all_events = all_events[all_events['event_type'].isin(event_types)]
...
def visualize_timeline(timeline, entity_id):
    if timeline.empty:
        return None
    fig = px.scatter(timeline, x='timestamp', y='event_type', color='event_type', hover_data=['location', 'text'], title=f'Activity Timeline for Entity: {entity_id}', labels={'timestamp': 'Timestamp', 'event_type': 'Event Type'}, height=600)
